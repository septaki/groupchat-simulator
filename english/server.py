...
# File mapping per agent (friendly key → actual filename)
def get_setting_files(agent_name: str) -> dict:
    # Maps friendly keys to actual filenames in agent directory
    common = {
    ...
    if any(x in agent_name for x in ["agent-1", "agent-2", "agent-3"]):
        return common
    return {}
...
class Agent:
    ...
    def build_system_prompt(self, other_names: list[str]) -> str:
        others = ", ".join(other_names) if other_names else "no one else"
        ...
        prompt = f"""You are {self.name}. Your persona:
...
---
# Important
- **At conversation start, first call read_setting_file(\"instructions\") to load your behavior rules, then read_setting_file(\"setting\") if needed.**
...
- Keep replies natural. Tool calls happen in the background — the user won't see them."""
    ...
    def read_setting_file(self, filename_key: str) -> str:
        ...
        if filename_key not in mapping:
            return f"Unknown file: {filename_key}. Available: {', '.join(mapping.keys())}"
        filepath = self.dir / mapping[filename_key]
        if not filepath.exists():
            return f"File not found: {mapping[filename_key]}"
        content = filepath.read_text(encoding="utf-8")
        if len(content) > 8000:
            head = content[:4000]
            tail = content[-3000:]
            return head + f"\n\n... ({len(content) - 7000} chars omitted) ...\n\n" + tail
        return content
    ...
    def get_emote(self, mood: str) -> dict:
        """Get emote: prefer image file, fallback to kaomoji + emoji"""
...
def verify_request(request: Request):
    """Verify Authorization token in request"""
    s = load_settings()
    if not s.get("password", ""):  # No password set, skip auth
        return True
...
PARTY_GM_PROMPT = """## Your Role: Game Master (GM)
...
async def call_agent(agent: Agent, conv: Conversation) -> dict:
    ...
    for _ in range(3):
        ...
        # Build assistant message (preserve reasoning_content for DeepSeek V4 Pro thinking mode)
...
async def websocket_endpoint(ws: WebSocket):
    ...
    async def party_gm_narrate(gm: Agent, c: Conversation):
        """GM narrates and presents choices. Returns True if waiting for user."""
        c.party_round += 1
        ...
        if is_first:
            instruction = get_party_gm_prompt(gm.name, c.party_choice_freq) + "\n\nStart round 1. Do NOT call any tools. You already have all the information you need. Create a world setting, opening scene, and first choice. Format at the end:\n【Options】\n1. Action description\n2. Action description\n3. Action description"
        elif is_choice_round:
            instruction = f"Round {c.party_round}. Give 3 options strictly in this format at the end:\n【Options】\n1. ...\n2. ...\n3. ..."
        else:
            instruction = f"Round {c.party_round}. Continue the story, 1-3 sentences. Give other players something to react to. No options."
    ...
    async def run_party_loop(gm: Agent, c: Conversation, gm_name: str):
        """Continue party until next choice round"""
        while not c.party_awaiting_choice:
            ...
            msgs = c.build_api_messages(gm)
            if is_choice_round:
                instruction = f"Round {c.party_round}. Give 3 options in format:\n【Options】\n1. ...\n2. ...\n3. ..."
            else:
    ...
    async def party_player_react(player: Agent, c: Conversation, gm_name: str):
        """Player agent reacts to GM narration"""
        await send_json({"type": "typing", "agent": player.name})

        msgs = c.build_api_messages(player)
        msgs.append({"role": "user", "content": get_party_player_prompt(gm_name) + "\n\nReact to what just happened in 1-2 sentences. Keep it short and natural, like chiming in a group chat. Stay in character."})
    ...
    async def end_session_reply(agent: Agent, c: Conversation):
        """End session: summarize, write memory, update state, farewell"""
        await send_json({"type": "typing", "agent": agent.name})
    ...
    try:
        while True:
            ...
            if msg_type == "user_message":
                ...
                online = get_online_agents()
                if len(online) < 3:
                    await send_json({"type": "error", "message": "Party mode requires at least 3 agents online (1 GM + 2 players). Please enable more agents first."})
                    continue
                ...
                conv.party_awaiting_choice = False
                await send_json({"type": "mode_changed", "mode": "party", "label": "Party Mode"})
